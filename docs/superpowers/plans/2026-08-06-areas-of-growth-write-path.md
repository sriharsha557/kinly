...
- **`goal_checkins` has no UPDATE policy, deliberately.** A supabase-js `.upsert()` takes the `ON CONFLICT DO UPDATE` path and **will be rejected by RLS with error 42501**. Check-ins must be written with `.insert(..., { ignoreDuplicates: true })`, which compiles to `ON CONFLICT DO NOTHING`.
