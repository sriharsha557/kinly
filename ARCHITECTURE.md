...
- **Illustrations**: custom SVG art lives in [assets/illustrations/](assets/illustrations/), imported directly as components via `react-native-svg-transformer` ([metro.config.js](metro.config.js), [svg.d.ts](svg.d.ts)) — the first raw `.svg` imports in the codebase; the app icon/`Logo` are still hand-coded `react-native-svg` primitives instead, unrelated to this pipeline. `GardenStageArt` ([src/components/GardenStageArt.tsx](src/components/GardenStageArt.tsx)) maps `GardenStage` (`seed`/`sprout`/`tree`/`bloom`) to real plant art, used by both `GardenCard` (per-member) and `GardenTeaser` (circle-level hero, picked by health tier). No `wilted` illustration exists yet — falls back to the 🥀 emoji. Also used: `kinly-Goal.svg` (target icon) replaces the 🎯 emoji on goal tags/chips in `ConnectionScreen`; `kinly-idea.svg` (a more detailed bulb illustration than `idea-bulb.svg`) heads `GoalSuggestions`' "Suggested for you" section.
