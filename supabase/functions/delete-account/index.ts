...
// files from Storage (a DB cascade never touches Storage objects), then
// removes the auth.users row via the Admin API - the one part that can only
// happen with the service role key, never from the client or a
...
const STORAGE_BUCKETS = ['avatars', 'vision-images'];
